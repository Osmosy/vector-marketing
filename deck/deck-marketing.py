...
DATA = dict(
    ...
    chips=[('19', 'агентов'), ('161', 'навыков'), ('40', 'PM-методик')],
    ...
    intro_cards=[
        ...
        ('Company Brain', ['8 файлов общего контекста:', 'голос бренда, anti-slop,', 'ICP, офферы, барьеры,', 'правовые границы']),
    ...
    layers_items=[
        ...
        ('Harness — что позволяет действовать', 'agents/*.md (19 ролей) + Hermes profile + 161 навык в 14 наборах + MCP-инструменты: Яндекс Метрика/Директ API, MPSTATS, ДaData, Wordstat, карты'),
    ...
    ag_items=[
        ...
        ('* ЗАГОТОВКА', 'профиль без навыков репозитория: роль описана, работать нечем. Разработаем позже: vk-ads, presentation'),
    ],
    ...
    # 10 · Roadmap
    roadmap_items=[
        ('Сейчас', ['40 PM-скиллов в 4 группах, 19 агентов, 161 навык', 'Живая архитектурная диаграмма (Archify) + визуальная верификация']),
...
def sl_agents(slide, th, D):
    ...
    y = 2.05
    # Пять панелей (пятая — «ЗАГОТОВКА»), а высот было четыре: сборка падала с
    # IndexError на пятом пункте, добавленном вместе с разделом про 38-ФЗ.
    # 5 * 0.85 + 4 * 0.12 = 4.73 → низ 6.78 при высоте слайда 7.5, футер не задет.
    hs = [0.85, 0.85, 0.85, 0.85, 0.85]
