...
### Шаг 3a: РФ — 152-ФЗ «О персональных данных» (адаптация Vector Marketing)

Если продукт работает с клиентами из РФ, по умолчанию применяется **152-ФЗ** (не GDPR):

- **Оператор ПДн**: компания, определяющая цели обработки — указать в политике наименование, ИНН, адрес
- **Правовые основания**: не «легитимный интерес» (это GDPR), а конкретные цели обработки + согласие субъекта (ст. 9) — письменное согласие на каждое согласованное действие
- **Форма согласия на лендинге**: чекбокс НЕ по умолчанию проставленный + ссылка на политику/согласие; текст согласия — конкретные действия с ПДн, не «иные цели»
- **Локализация (ч. 5 ст. 18)**: первичный сбор ПДн граждан РФ — в базах на территории РФ (важно для выбора хостинга и CRM)
- **Уведомление Роскомнадзора**: оператор должен быть включён в реестр операторов ПДн — проверить до запуска
- **Права субъекта**: уточнение/блокировка/уничтожение данных (ст. 14) — процедура и срок ответа (10 рабочих дней)
- **Утечки**: уведомление Роскомнадзора в течение 24 часов (осведомлённость) + 72 часов (детали)
- **Cookies/аналитика**: Метрика/Директ — указать в политике как обработчики
- **Дети**: согласие родителя для лиц до 18 лет (точнее — до возраста согласия по 152-ФЗ нет отдельного порога, но для аудитории <18 учитывать)

**Sanctions/two-jurisdictions**: если клиент работает и с РФ, и с ЕС — политика должна разделять режимы, не смешивать. GDPR применяется к EU-посетителям параллельно.
...
Скилл заимствован из [PM Skills Marketplace](https://github.com/phuryn/pm-skills) (MIT, куратор Павел Хурын, The Product Compass). Используется агентом landing-cro экосистемы Vector Marketing.
