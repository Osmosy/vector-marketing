...
## Проверка для рынка РФ (адаптация Vector Marketing)

Для российских продуктов добавляйте к каждому имени:
- **Товарные знаки:** проверка по базе Роспатента (ФИПС, открытые реестры). Отметить классы МКТУ, где возможно пересечение
- **Домены:** .ru и .рф (кириллический вариант — проверить отдельно, кириллица меняет произношение и восприятие)
- **Загрязнённые смыслы:** проверка значения на основных языках РФ и СНГ (не только английский), сленг, ассоциации
- **Юрдичество:** если имя совпадает с зарегистрированным брендом в том же классе МКТУ — flagged как risk, не предлагать
- **Паттерны рынка:** в РФ часто работают транслит-имена (Yandex, Avito), русские корни (Тинькофф, Wildberries) и описательные имена — учитывать все три модели при генерации
