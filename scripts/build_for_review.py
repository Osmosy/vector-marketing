...
def build() -> str:
    ...
    # Ссылка на прогон, а не «прогнано локально»: локальный прогон внешнему
    ...
    lines = [
        ...
        "",
        # Раньше здесь стояло `Коммит: \`{commit}\`` — хеш HEAD на момент генерации.
        # Это ложь по построению: файл генерируется из рабочего дерева, которое затем
        # и коммитится вместе с ним, поэтому хеши описывают дерево ТОГО коммита, где
        # файл лежит, а названный хеш всегда на один коммит позади (проверено: в
        # for-review от fec4f54 хеши совпадали с деревом 51bdddd, и наоборот).
        # Называть хеш, которого в цитируемой ревизии ещё нет, — хуже, чем не называть
        # его вовсе, поэтому здесь только правило и команда его проверки.
        "Хеши ниже описывают дерево той ревизии, в которой лежит сам этот файл "
        "(он генерируется из того же дерева, что и коммитится, — поэтому хеш HEAD "
        "на момент сборки здесь называть нельзя: он на один коммит позади). "
        "Ревизию получить так: `git log -1 --format=%H -- docs/for-review.md`. "
        "Если присланный архив совпадает по хешам, он из этой ревизии, а не из кеша.",
        "",
        "## Файлы",
        "",
        "| Файл | Байт | sha256 |",
